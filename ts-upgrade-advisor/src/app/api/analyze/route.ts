...
Return your analysis as a JSON object with this exact structure:
{
  ...
  "sdkVersionWarning": "null or a string if their SDK version is behind the recommended",
  "issues": [
    {
      "id": "unique-id",
  ...
  "emailDraft": {
    ...
    "body": "Full email body in plain text, addressed to the dev team, explaining the upgrade impacts and required actions"
  }
}
...
5. Any new features introduced in the upgrade range that are relevant to their embed type (AppEmbed/LiveboardEmbed/SpotterEmbed etc)

Be precise — only flag what is actually in their code.`

    // Call Claude API with customer's own key
    ...
    try {
      ...
      return NextResponse.json({ error: 'Failed to parse analysis. Raw response: ' + rawText.substring(0, 500) }, { status: 500 })
    }
