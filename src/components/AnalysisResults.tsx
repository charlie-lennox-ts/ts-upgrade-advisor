...
interface Issue {
  ...
  fromPreviewNotes?: boolean
}
interface Opportunity { title: string; detail: string; docsLink?: string; fromPreviewNotes?: boolean }
interface Analysis {
...
const DOCS_LINKS: Record<string, string> = {
  'breaking-change': 'https://developers.thoughtspot.com/docs/embed-sdk-changelog',
  'deprecation':     'https://developers.thoughtspot.com/docs/embed-sdk-changelog',
  'css-variable':    'https://developers.thoughtspot.com/docs/custom-css',
  'sdk-version':     'https://developers.thoughtspot.com/docs/whats-new',
  'new-feature':     'https://developers.thoughtspot.com/docs/whats-new',
  'default':         'https://developers.thoughtspot.com/docs/embed-sdk-changelog',
}

function getDocsLink(category: string): string {
  return DOCS_LINKS[category] || DOCS_LINKS['default']
}

function DraftPill() {
  return (
    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs font-medium"
          style={{ background: 'rgba(113,75,251,0.12)', color: '#A78BFA', border: '1px solid rgba(113,75,251,0.2)', fontSize: '10px' }}
          title="Based on draft preview release notes — subject to change before GA">
      ✦ draft
    </span>
  )
}
...
function IssueCard({ issue }: { issue: Issue }) {
  ...
  const docsLink = getDocsLink(issue.category)
    ...
    <div className="rounded-lg overflow-hidden" style={{ border: `1px solid rgba(255,255,255,0.06)`, borderLeft: `2px solid ${cfg.border}`, background: cfg.bg }}>
        ...
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {issue.fromPreviewNotes && <DraftPill />}
            </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-3" style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 12 }}>
          ...
          {issue.fromPreviewNotes && (
            <p className="text-xs" style={{ color: '#7AA8C4', fontStyle: 'italic' }}>
              ✦ This finding is based on draft preview release notes for an upcoming release and may change before general availability. Verify against official release notes before acting.
            </p>
          )}
          <a href={docsLink} target="_blank" rel="noopener noreferrer"
             className="inline-flex items-center gap-1.5 text-xs hover:underline" style={{ color: '#04D1FF' }}>
            <ExternalLink size={11} /> View in SDK changelog
          </a>
        </div>
      )}
    </div>
  ...
  const handleCopyAnalysis = () => {
    const lines = [
      ...
      ...analysis.issues
        ...
        .map(i => `[${i.severity.toUpperCase()}] ${i.title}${i.fromPreviewNotes ? ' [DRAFT]' : ''}\n${i.detail}${i.fix ? `\nAction: ${i.fix}` : ''}`),
    ]
  ...
  return (
    <div className="space-y-4 animate-fadeIn">
      ...
      <div className="ts-card p-5">
        ...
        <p className="text-xs mt-3" style={{ color: '#3A5572' }}>
          Findings are based on live ThoughtSpot docs fetched on this run. Always verify against official release notes before upgrading.
        </p>
      </div>
            ...
            {analysis.opportunities.map((opp, i) => (
              <div key={i} className="rounded-lg p-4" style={{ border: '1px solid rgba(255,255,255,0.06)', borderLeft: '2px solid rgba(4,209,255,0.4)', background: 'rgba(4,209,255,0.04)' }}>
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  {opp.fromPreviewNotes && <DraftPill />}
                </div>
                <p className="text-xs leading-relaxed" style={{ color: '#7AA8C4' }}>{opp.detail}</p>
                {opp.fromPreviewNotes && (
                  <p className="text-xs mt-2" style={{ color: '#7AA8C4', fontStyle: 'italic' }}>
                    ✦ Based on draft preview notes — verify before acting.
                  </p>
                )}
                <a href={getDocsLink('new-feature')} target="_blank" rel="noopener noreferrer"
                   className="inline-flex items-center gap-1 text-xs mt-2 hover:underline" style={{ color: '#04D1FF' }}>
                  <ExternalLink size={11} /> View in What's New
                </a>
              </div>
            ))}
          </div>
