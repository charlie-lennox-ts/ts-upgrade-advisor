...
export default function CodeInput({ code, onCodeChange, githubUrl, onGithubUrlChange, onSdkDetected }: CodeInputProps) {
  ...
  const processZip = async (file: File) => {
    ...
    setFileError(null)
    try {
      // Use JSZip via CDN-style dynamic import workaround — read as ArrayBuffer
      ...
      // Prioritise package.json and embed-related files
      ...
      {mode === 'file' && (
        <div className="space-y-3">
          ...
          {!fileName ? (
            ...
            >
              {processing ? <><div className="w-5 h-5 border-2 border-current border-t-transparent rounded-full animate-spin" /><span className="text-sm">Processing zip…</span></> : (
                <>
                  <Upload size={22} />
            ...
            <div className="flex items-center justify-between px-4 py-3 rounded-lg" style={{ background: 'rgba(109,210,103,0.08)', border: '1px solid rgba(109,210,103,0.2)' }}>
              ...
              <button onClick={() => { setFileName(null); handleCodeChange(''); if (fileRef.current) fileRef.current.value = '' }} style={{ color: '#3A5572' }}><X size={16} /></button>
            </div>
          )}
          ...
          <div className="rounded-lg p-3 space-y-2" style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(4,209,255,0.08)' }}>
            ...
            <p className="text-xs mt-2" style={{ color: '#3A5572' }}>
              If uploading a zip, these will be auto-extracted. The following are ignored: {IGNORE_FILES.join(', ')}
            </p>
          </div>
        </div>
      )}
      ...
      {mode === 'github' && (
        <div className="space-y-3">
          <div className="flex items-start gap-2 rounded-lg px-3 py-2.5" style={{ background: 'rgba(255,192,82,0.08)', border: '1px solid rgba(255,192,82,0.2)' }}>
            <AlertTriangle size={13} className="shrink-0mt-0.5" style={{ color: '#FFC052' }} />
          ...
          <div className="rounded-lg p-3 space-y-2" style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(4,209,255,0.08)' }}>
            ...
            <p className="text-xs mt-2" style={{ color: '#3A5572' }}>Fetched fresh on every analysis run. Paste a github.com file URL — we convert it to raw automatically.</p>
          </div>
